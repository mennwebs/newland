...
if ( ! function_exists( 'seed_posted_cats' ) ) :
	...
	function seed_posted_cats($show_icon = true) {
		if ( 'post' === get_post_type() ) {
			$categories_list = get_the_category_list( esc_html__( ', ', 'newland' ) );
			if ( $categories_list ) {
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'newland' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
<div class="site-member">
    <a href="<?php echo $GLOBALS['s_member_url']; ?>" class="m-user">
        <?php 
				...
				if( 0 != $current_user->ID) { echo get_avatar($current_user->ID, 64 ); } else { 
					echo '<svg fill="none" height="32" width="32" xmlns="http://www.w3.org/2000/svg"><path d="m0 16c0-8.837 7.163-16 16-16s16 7.163 16 16-7.163 16-16 16-16-7.163-16-16z" fill="#fff"/><g fill="#161616"><path clip-rule="evenodd" d="m16 8.5a3.125 3.125 0 1 1 0 6.25 3.125 3.125 0 0 1 0-6.25zm0-1.25a4.375 4.375 0 1 0 0 8.75 4.375 4.375 0 0 0 0-8.75zm6.25 17.5h-1.25v-3.125a3.125 3.125 0 0 0 -3.125-3.125h-3.75a3.125 3.125 0 0 0 -3.125 3.125v3.125h-1.25v-3.125a4.375 4.375 0 0 1 4.375-4.375h3.75a4.375 4.375 0 0 1 4.375 4.375z" fill-rule="evenodd"/><path d="m16 31c-8.284 0-15-6.716-15-15h-2c0 9.389 7.611 17 17 17zm15-15c0 8.284-6.716 15-15 15v2c9.389 0 17-7.611 17-17zm-15-15c8.284 0 15 6.716 15 15h2c0-9.389-7.611-17-17-17zm0-2c-9.389 0-17 7.611-17 17h2c0-8.284 6.716-15 15-15z"/></g></svg>';
				} 
			?>
    </a>
</div>
...
function seed_banner_title($post_id) {
	...
	if( is_front_page() ) { 
		...
		$title = __('Page not found', 'newland' ); 
	} else { 
